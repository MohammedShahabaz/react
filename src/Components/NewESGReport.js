...
export default function NewESGReport() {
  const [open, setOpen] = React.useState(false);
  const [anchorEl, setAnchorEl] = React.useState(null);
  const [placement, setPlacement] = React.useState();
  ...
  function dummyFunction() {
    // console.log("call")
    axios.get("http://localhost:3000/organization/" + cik).then((response) => {
  ...
  const renderHTML = () => {
    ...
    location_list.forEach((element) => {
      html.push(
        ...
        >
          <Card
            sx={{ width: 275, height: 200, borderWidth: 1 }}
            variant="outlined"
          >
            <CardContent>
  ...
  const handleAddClick = () => {
    ...
    axios
      .post(
        "http://localhost:3000/location/" + cik,
        {
          country: newCountry,
          street: newStreet,
          city: newCity,
      ...
      >
        ...
        >
          ...
          <Autocomplete
            ...
            clearOnEscape
            options={
              name_for_autocomplete.length > 0 ? name_for_autocomplete : []
            }
            value={
              name_for_autocomplete.length > 0
                ? name_for_autocomplete.filter(
                    (item) => item.name === name_for_autocomplete[0].name
                  )[0]
                : []
            }
